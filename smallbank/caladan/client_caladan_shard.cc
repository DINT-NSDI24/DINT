  ...
  while (true) {
    ...
    switch (txn_type) {
      case TxnType::kAmalgamate:
        txn_committed = TxnAmalgamate(wrkr_lid, txn_suc_pkt_cnt, tg_seed, conns, servaddr);
        break;
      case TxnType::kBalance:
        txn_committed = TxnBalance(wrkr_lid, txn_suc_pkt_cnt, tg_seed, conns, servaddr);
        break;
      case TxnType::kDepositChecking:
        txn_committed = TxnDepositChecking(wrkr_lid, txn_suc_pkt_cnt, tg_seed, conns, servaddr);
        break;
      case TxnType::kSendPayment:
        txn_committed = TxnSendPayment(wrkr_lid, txn_suc_pkt_cnt, tg_seed, conns, servaddr);
        break;
      case TxnType::kTransactSaving:
        txn_committed = TxnTransactSaving(wrkr_lid, txn_suc_pkt_cnt, tg_seed, conns, servaddr);
        break;
      case TxnType::kWriteCheck:
        txn_committed = TxnWriteCheck(wrkr_lid, txn_suc_pkt_cnt, tg_seed, conns, servaddr);
        break;
      default:
