...
static inline void kvs_get(kvs *kvs, uint64_t key, uint8_t *val, uint32_t *ver) {
  ...
  panic("kvs_get: key not found");
}

static inline void kvs_set(kvs *kvs, uint64_t key, uint8_t *val) {
  uint32_t hash = kvs_hash(kvs, key);
  while (__sync_lock_test_and_set(&kvs->locks[hash], 1));
