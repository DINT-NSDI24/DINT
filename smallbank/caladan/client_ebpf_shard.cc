...
void WarmupThread(int wid) {
  uint32_t kSliceSize = kAccountNum / threads;
  uint32_t l_sid = wid * kSliceSize;
  uint32_t r_sid = (wid == threads - 1) ? kAccountNum : (wid + 1) * kSliceSize;
  ...
  for (uint32_t acct_id = l_sid; acct_id < r_sid; acct_id++) {
    // Savings
    sb_sav_key_t sav_key;
    sav_key.acct_id = (uint64_t)acct_id;

    msg.key = *(uint64_t *)&sav_key;
    msg.table = TableType::kSaving;

    while (true) {
      msg.type = PktType::kWarmupRead;
      NetHandshake(&msg, c.get(), servaddr[msg.key%3]);
      assert(msg.key == *(uint64_t *)&sav_key);
      assert(msg.table == TableType::kSaving);
      if (msg.type == PktType::kWarmupReadAck) break;
      else assert(msg.type == PktType::kRetry);
    }
    ...
    msg.key = *(uint64_t *)&chk_key;
    msg.table = TableType::kChecking;

    while (true) {
      msg.type = PktType::kWarmupRead;
      NetHandshake(&msg, c.get(), servaddr[msg.key%3]);
      assert(msg.key == *(uint64_t *)&chk_key);
      assert(msg.table == TableType::kChecking);
      if (msg.type == PktType::kWarmupReadAck) break;
      else assert(msg.type == PktType::kRetry);
    }
  ...
  log_emerg("worker %d warmup ebpf cache done", wid);
}
...
void ClientHandler(void *arg) {
  if (machine_id == 1) {
    ...
    std::vector<rt::Thread> warmup_th;
    for (int i = 0; i < threads; i++) {
      warmup_th.emplace_back(rt::Thread([&, i]{
        WarmupThread(i);
      }));
    }

    for (auto &t: warmup_th)
      t.Join();
