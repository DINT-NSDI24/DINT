...
int tps_prim_xdp_main(struct xdp_md *ctx) {
  ...
  if (msg->type == READ) {
    ...
    uint64_t ret = __sync_val_compare_and_swap(&(lock->lock_bit), 0, 1);
    if (ret == 0) {
      msg->type = GRANT_LOCK;
      lock->key = msg->key;
      ...
      if (lock->key != msg->key) msg->type = REJECT_LOCK;
      else msg->type = REJECT_LOCK_SAME_KEY;
    ...
    __sync_val_compare_and_swap(&(lock->lock_bit), 1, 0);
    msg->type = ABORT_ACK;
