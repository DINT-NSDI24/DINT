...
std::vector<uint64_t> lock_cnt, reject_sharing_cnt, reject_same_key_cnt;
std::atomic<bool> stat_started{false};

// create workload generator array
void CreateWorkgenArr() {
  workgen_arr.reserve(100);
  workgen_arr.insert(workgen_arr.end(), kFreqGetSubscriberData,
                     TxnType::kGetSubscriberData);
  workgen_arr.insert(workgen_arr.end(), kFreqGetAccessData,
                     TxnType::kGetAccessData);
  workgen_arr.insert(workgen_arr.end(), kFreqGetNewDestination,
                     TxnType::kGetNewDestination);
  workgen_arr.insert(workgen_arr.end(), kFreqUpdateSubscriberData,
                     TxnType::kUpdateSubscriberData);
  workgen_arr.insert(workgen_arr.end(), kFreqUpdateLocation,
                     TxnType::kUpdateLocation);
  workgen_arr.insert(workgen_arr.end(), kFreqInsertCallForwarding,
                     TxnType::kInsertCallForwarding);
  workgen_arr.insert(workgen_arr.end(), kFreqDeleteCallForwarding,
                     TxnType::kDeleteCallForwarding);
...
void PopulateThread(int wid) {
  ...
  uint32_t r_sid =
      (wid == threads - 1) ? kSubscriberNum : (wid + 1) * kSliceSize;
  ...
  for (uint32_t s_id = l_sid; s_id < r_sid; s_id++) {
    ...
    for (int i = 0; i < 5; i++) sub_val.hex[i] = fastrand(&tmp_seed);

    for (int i = 0; i < 10; i++) sub_val.bytes[i] = fastrand(&tmp_seed);
    ...
    while (true) {
      ...
      if (msg.type == PktType::kInsertPrimAck)
        break;
      else
  ...
  for (uint32_t s_id = l_sid; s_id < r_sid; s_id++) {
    std::vector<uint8_t> ai_type_vec =
        select_between_n_and_m_from(tmp_seed, ai_type_values, 1, 4);
  ...
  for (uint32_t s_id = l_sid; s_id < r_sid; s_id++) {
    std::vector<uint8_t> sf_type_vec =
        select_between_n_and_m_from(tmp_seed, sf_type_values, 1, 4);
  ...
  log_emerg(
      "worker %d populate special_facility table and call_forwarding table "
      "done",
      wid);
}
...
#define CPU_MON_PORT 20231
  ...
  uint64_t total_suc_txn =
      std::accumulate(suc_txn_cnt.begin(), suc_txn_cnt.end(), 0UL);
  uint64_t total_lock = std::accumulate(lock_cnt.begin(), lock_cnt.end(), 0UL);
  uint64_t total_reject_sharing = std::accumulate(
      reject_sharing_cnt.begin(), reject_sharing_cnt.end(), 0UL);
  uint64_t total_reject_same_key = std::accumulate(
      reject_same_key_cnt.begin(), reject_same_key_cnt.end(), 0UL);
  ...
  log_emerg("total reject ratio due to lock sharing: %lf",
            (double)total_reject_sharing / total_lock);
  log_emerg("total reject ratio due to same key: %lf",
            (double)total_reject_same_key / total_lock);
}
...
void PrintTput(uint32_t poll_cnt) {
  ...
  static uint64_t last_lock = 0;
  static uint64_t last_reject_sharing = 0;
  static uint64_t last_reject_same_key = 0;
  ...
  log_emerg("lock: %lu", total_lock - last_lock);
  log_emerg("reject_sharing: %lu", total_reject_sharing - last_reject_sharing);
  log_emerg("reject_same_key: %lu",
            total_reject_same_key - last_reject_same_key);

  last_txn = total_txn;
  last_pkt = total_pkt;
  last_suc_txn = total_suc_txn;
  last_suc_pkt = total_suc_pkt;
  last_lock = total_lock;
  last_reject_sharing = total_reject_sharing;
  last_reject_same_key = total_reject_same_key;
}
...
void StatLoop() {
  ...
  while (1) {
    ...
    poll_cnt++;
    if (unlikely(poll_cnt == kStatsStartSec + 1)) stat_started = true;
...
bool TxnGetSubscriberData(int worker_id, uint64_t &txn_suc_pkt_cnt,
                          uint64_t &tg_seed,
                          const std::vector<rt::UdpConn *> &conns) {
  tatp_sub_key_t key;
  ...
  tatp_sub_val_t *val = (tatp_sub_val_t *)&msg.val;
  _unused(val);
...
bool TxnGetNewDestination(int worker_id, uint64_t &txn_suc_pkt_cnt,
  ...
  while (true) {
    ...
    NetHandshake(&specfac_msg, conns[specfac_msg.key % 3],
                 servaddr[specfac_msg.key % 3]);
    assert(specfac_msg.key == *(uint64_t *)&specfac_key);
    assert(specfac_msg.table == TableType::kSpecialFacility);
    txn_suc_pkt_cnt++;
    if (specfac_msg.type == PktType::kGrantRead ||
        specfac_msg.type == PktType::kNotExist)
      break;
    else
      assert(specfac_msg.type == PktType::kRejectRead);
  }
...
bool TxnGetAccessData(int worker_id, uint64_t &txn_suc_pkt_cnt,
  ...
  // the key was found
  tatp_accinf_val_t *val = (tatp_accinf_val_t *)&msg.val;
  _unused(val);
...
bool TxnUpdateSubscriberData(int worker_id, uint64_t &txn_suc_pkt_cnt,
  ...
  if (stat_started) lock_cnt[worker_id] += 2;
  ...
  if (stat_started) {
    reject_sharing_cnt[worker_id] +=
        (sub_msg_lock.type == PktType::kRejectLock) ? 1 : 0;
    reject_sharing_cnt[worker_id] +=
        (specfac_msg_lock.type == PktType::kRejectLock) ? 1 : 0;
    reject_same_key_cnt[worker_id] +=
        (sub_msg_lock.type == PktType::kRejectLockSameKey) ? 1 : 0;
    reject_same_key_cnt[worker_id] +=
        (specfac_msg_lock.type == PktType::kRejectLockSameKey) ? 1 : 0;
  }

  assert(sub_msg_read.type == PktType::kGrantRead);

  if (specfac_msg_read.type == PktType::kNotExist ||
      sub_msg_lock.type == PktType::kRejectLock ||
    ...
    if (sub_msg_lock.type == PktType::kGrantLock) {
      ...
      NetHandshake(&sub_msg_lock, conns[sub_msg_lock.key % 3],
                   servaddr[sub_msg_lock.key % 3]);

      assert(sub_msg_lock.type == PktType::kAbortAck);
      txn_suc_pkt_cnt++;
    } else
      assert(sub_msg_lock.type == PktType::kRejectLock);

    // release lock on special facility record
    if (specfac_msg_lock.type == PktType::kGrantLock) {
      specfac_msg_lock.type = PktType::kAbort;
      NetHandshake(&specfac_msg_lock, conns[specfac_msg_lock.key % 3],
                   servaddr[specfac_msg_lock.key % 3]);

      assert(specfac_msg_lock.type == PktType::kAbortAck);
      txn_suc_pkt_cnt++;
    } else
      assert(specfac_msg_lock.type == PktType::kRejectLock);

    return false;
  } else
    assert(specfac_msg_read.type == PktType::kGrantRead &&
           sub_msg_lock.type == PktType::kGrantLock &&
  ...
  if (sub_msg_read.ver != sub_msg_ver.ver ||
      specfac_msg_read.ver != specfac_msg_ver.ver) {
    // abort
    sub_msg_lock.type = PktType::kAbort;
    NetHandshake(&sub_msg_lock, conns[sub_msg_lock.key % 3],
                 servaddr[sub_msg_lock.key % 3]);
  ...
  for (int i = 0; i < 3; ++i) {
    shard_wrkrs.emplace_back(rt::Thread([&, i]() {
      for (auto &msg : shard_msgs[i]) NetSend(msg, conns[i], servaddr[i]);
...
bool TxnUpdateLocation(int worker_id, uint64_t &txn_suc_pkt_cnt,
  ...
  while (true) {
    ...
    NetHandshake(&sec_sub_msg, conns[sec_sub_msg.key % 3],
                 servaddr[sec_sub_msg.key % 3]);
    assert(sec_sub_msg.key == *(uint64_t *)&sec_sub_key);
    assert(sec_sub_msg.table == TableType::kSecondSubscriber);
    txn_suc_pkt_cnt++;
    if (sec_sub_msg.type == PktType::kGrantRead)
      break;
    else
      assert(sec_sub_msg.type == PktType::kRejectRead);
  }
  ...
  if (stat_started) lock_cnt[worker_id]++;
  ...
  assert(sub_msg_read.type == PktType::kGrantRead);
  if (sub_msg_lock.type == PktType::kRejectLock)
    return false;
  else
    assert(sub_msg_lock.type == PktType::kGrantLock);
  ...
  while (1) {
    sub_msg_ver.type = PktType::kRead;
    NetHandshake(&sub_msg_ver, conns[sub_msg_ver.key % 3],
                 servaddr[sub_msg_ver.key % 3]);
    assert(sub_msg_ver.key == *(uint64_t *)&sub_key);
    assert(sub_msg_ver.table == TableType::kSubscriber);
    txn_suc_pkt_cnt++;
    if (sub_msg_ver.type == PktType::kGrantRead)
      break;
    else
      assert(sub_msg_ver.type == PktType::kRejectRead);
  }
  ...
  while (true) {
    sub_msg_read.type = PktType::kCommitPrim;
    NetHandshake(&sub_msg_read, conns[sub_msg_read.key % 3],
                 servaddr[sub_msg_read.key % 3]);
    txn_suc_pkt_cnt++;
    if (sub_msg_read.type == PktType::kCommitPrimAck)
      break;
    else
      assert(sub_msg_read.type == PktType::kRejectCommit);
  }
...
bool TxnInsertCallForwarding(int worker_id, uint64_t &txn_suc_pkt_cnt,
  ...
  while (true) {
    specfac_msg_read.type = PktType::kRead;
    NetHandshake(&specfac_msg_read, conns[specfac_msg_read.key % 3],
                 servaddr[specfac_msg_read.key % 3]);
    assert(specfac_msg_read.key == *(uint64_t *)&specfac_key);
    assert(specfac_msg_read.table == TableType::kSpecialFacility);
    txn_suc_pkt_cnt++;
    if (specfac_msg_read.type == PktType::kGrantRead ||
        specfac_msg_read.type == PktType::kNotExist)
      break;
    else
      assert(specfac_msg_read.type == PktType::kRejectRead);
  }
  ...
  if (stat_started) {
    reject_sharing_cnt[worker_id] +=
        (callfwd_msg_lock.type == PktType::kRejectLock) ? 1 : 0;
    reject_same_key_cnt[worker_id] +=
        (callfwd_msg_lock.type == PktType::kRejectLockSameKey) ? 1 : 0;
  }

  if (callfwd_msg_read.type == PktType::kGrantRead ||
      callfwd_msg_lock.type == PktType::kRejectLock) {
    // release lock on call forwarding record
    if (callfwd_msg_lock.type == PktType::kGrantLock) {
      callfwd_msg_lock.type = PktType::kAbort;
      NetHandshake(&callfwd_msg_lock, conns[callfwd_msg_lock.key % 3],
                   servaddr[callfwd_msg_lock.key % 3]);

      assert(callfwd_msg_lock.type == PktType::kAbortAck);
      txn_suc_pkt_cnt++;
    } else
      assert(callfwd_msg_lock.type == PktType::kRejectLock);

    return false;
  } else
    assert(callfwd_msg_read.type == PktType::kNotExist &&
           callfwd_msg_lock.type == PktType::kGrantLock);
  ...
  if (specfac_msg_read.ver != specfac_msg_ver.ver ||
      callfwd_msg_ver.type == PktType::kGrantRead) {
    // abort
    callfwd_msg_lock.type = PktType::kAbort;
    NetHandshake(&callfwd_msg_lock, conns[callfwd_msg_lock.key % 3],
                 servaddr[callfwd_msg_lock.key % 3]);
    ...
    assert(callfwd_msg_ver.type == PktType::kNotExist);
  ...
  while (true) {
    ...
    NetHandshake(&callfwd_msg_read, conns[callfwd_msg_read.key % 3],
                 servaddr[callfwd_msg_read.key % 3]);
    txn_suc_pkt_cnt++;
    if (callfwd_msg_read.type == PktType::kInsertPrimAck)
      break;
    else
      assert(callfwd_msg_read.type == PktType::kRejectCommit);
  }
  return true;
}
...
bool TxnDeleteCallForwarding(int worker_id, uint64_t &txn_suc_pkt_cnt,
  ...
  if (callfwd_msg_read.type == PktType::kNotExist ||
      callfwd_msg_lock.type == PktType::kRejectLock) {
    // release lock on call forwarding record
    if (callfwd_msg_lock.type == PktType::kGrantLock) {
      callfwd_msg_lock.type = PktType::kAbort;
    ...
    assert(callfwd_msg_read.type == PktType::kGrantRead &&
           callfwd_msg_lock.type == PktType::kGrantLock);
  ...
  while (true) {
    callfwd_msg_ver.type = PktType::kRead;
    NetHandshake(&callfwd_msg_ver, conns[callfwd_msg_ver.key % 3],
                 servaddr[callfwd_msg_ver.key % 3]);
    assert(callfwd_msg_ver.key == *(uint64_t *)&callfwd_key);
    assert(callfwd_msg_ver.table == TableType::kCallForwarding);
    txn_suc_pkt_cnt++;
    if (callfwd_msg_ver.type == PktType::kGrantRead ||
        callfwd_msg_ver.type == PktType::kNotExist)
      break;
    else
      assert(callfwd_msg_ver.type == PktType::kRejectRead);
  }

  if (callfwd_msg_ver.type == PktType::kNotExist ||
      callfwd_msg_ver.ver != callfwd_msg_read.ver) {
    ...
    assert(callfwd_msg_ver.type == PktType::kGrantRead);
  ...
  while (true) {
    ...
    NetHandshake(&callfwd_msg_read, conns[callfwd_msg_read.key % 3],
                 servaddr[callfwd_msg_read.key % 3]);
    txn_suc_pkt_cnt++;
    if (callfwd_msg_read.type == PktType::kDeletePrimAck)
      break;
    else
      assert(callfwd_msg_read.type == PktType::kRejectCommit);
  }
  ...
  while (true) {
    ...
    switch (txn_type) {
      case TxnType::kGetSubscriberData:
        txn_committed =
            TxnGetSubscriberData(wrkr_lid, txn_suc_pkt_cnt, tg_seed, conns);
        break;
      case TxnType::kGetNewDestination:
        txn_committed =
            TxnGetNewDestination(wrkr_lid, txn_suc_pkt_cnt, tg_seed, conns);
        break;
      case TxnType::kGetAccessData:
        txn_committed =
            TxnGetAccessData(wrkr_lid, txn_suc_pkt_cnt, tg_seed, conns);
        break;
      case TxnType::kUpdateSubscriberData:
        txn_committed =
            TxnUpdateSubscriberData(wrkr_lid, txn_suc_pkt_cnt, tg_seed, conns);
        break;
      case TxnType::kUpdateLocation:
        txn_committed =
            TxnUpdateLocation(wrkr_lid, txn_suc_pkt_cnt, tg_seed, conns);
        break;
      case TxnType::kInsertCallForwarding:
        txn_committed =
            TxnInsertCallForwarding(wrkr_lid, txn_suc_pkt_cnt, tg_seed, conns);
        break;
      case TxnType::kDeleteCallForwarding:
        txn_committed =
            TxnDeleteCallForwarding(wrkr_lid, txn_suc_pkt_cnt, tg_seed, conns);
        break;
      default:
...
void ClientHandler(void *arg) {
  if (machine_id == 1) {
    ...
    for (int i = 0; i < threads; i++) {
      pop_th.emplace_back(rt::Thread([&, i] { PopulateThread(i); }));
    }

    for (auto &t : pop_th) t.Join();
  ...
  for (int i = 0; i < threads; i++) {
    ...
    th.emplace_back(rt::Thread([&, wrkr_gid] { ClientLoop(wrkr_gid); }));
  }
...
int main(int argc, char **argv) {
  if (argc != 7) {
    std::cerr << "usage: [cfg_file] [machine_id] [#clients] [#threads] "
                 "[target_load] [debug/expr]"
              << std::endl;
    return -EINVAL;
  }
  ...
  if (target_load == 0)
    ...
    net_intv = machine_num * threads * 1000000UL / target_load;
  ...
  lock_cnt.resize(threads, 0);
  reject_sharing_cnt.resize(threads, 0);
  reject_same_key_cnt.resize(threads, 0);
