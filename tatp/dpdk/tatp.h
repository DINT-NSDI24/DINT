...
static inline int lock_hash(kvs *kvs, uint64_t key) {
  return (int)(fasthash64(&key, sizeof(key), 0xdeadbeef) %
               (uint64_t)(kKeysPerEntry * kvs->hash_size));
}
...
static inline std::vector<uint8_t> select_between_n_and_m_from(
    uint64_t &tmp_seed, const std::vector<uint8_t> &values, unsigned N,
    unsigned M) {
  assert(M >= N);
...
static inline void populate_specfac_and_callfwd_table(kvs *specfac_table,
                                                      kvs *callfwd_table) {
